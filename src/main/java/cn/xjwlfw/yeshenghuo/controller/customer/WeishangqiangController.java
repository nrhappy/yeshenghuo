...
public class WeishangqiangController extends BaseController {
	...
	public String weixinIndex(HttpServletRequest request, Model model) {
		...
		WechatUser wechatUser = wechatService.getWechatUserInfoByOpenId(openId);
...
//		WechatUser user = wechatService.getUserWithoutSubscribe(map.get("access_token"), map.get("openid"), businessId);
		return weishangqiangService.index(wechatUser.getId() , businessId, model);
	}
	...
	@ResponseBody
	public Result ownScreenList(@RequestParam(value = "weishangqiangId") int weishangqiangId) {
		return weishangqiangService.ownScreenList(weishangqiangId);
	}
	...
	public Result createOrder(HttpServletRequest request,
			...
			@RequestParam(value = "businessId") int businessId,
			@RequestParam(value = "weishangqiangId") int weishangqiangId,
			@RequestParam(value = "goodsType") short goodsType) {
		return weishangqiangService.createOrder(request, wechatUserId, ownScreenId, businessId, weishangqiangId, goodsType);
	}
	 ...
	 * @since 2017年4月4日
	 */
	@RequestMapping(value = "/setWeishangqiangOn")
	@ResponseBody
	public Result setWeishangqiangOn(@RequestParam(value = "weishangqiangId") int weishangqiangId) {
		return weishangqiangService.setWeishangqiangOn(weishangqiangId);
	}
	 ...
	 * @since 2017年4月4日
	 */
	@RequestMapping(value = "/setWeishangqiangOff")
	@ResponseBody
	public Result setWeishangqiangOff(@RequestParam(value = "weishangqiangId") int weishangqiangId) {
		return weishangqiangService.setWeishangqiangOff(weishangqiangId);
	}
