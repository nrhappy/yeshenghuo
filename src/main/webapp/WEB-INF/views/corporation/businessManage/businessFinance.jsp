...
<html lang="zh-CN">
  ...
  <body style="padding:15px; overflow:auto;">
    ...
    <%-- <form role="form row" action="<%=request.getContextPath()%>/corporation/agencyManage/agencyInfo" method="get" onsubmit="return check()">
      <div class="form-group input-group">
        <input id="agencyName" name="agencyName" class="form-control pull-right" style="width:25%;" type="text" placeholder="请输入代理商名称">
    ...
    <table class="table table-bordered table-hover">
      ...
      <c:forEach items="${businessOrderList}" var="order" varStatus="status">
      <tr>
        <td>${order.id}</td>
        <td>${order.businessName}</td>
        <td>
          <c:if test="${order.goodsType == 0}">霸屏</c:if>
          <c:if test="${order.goodsType == 1}">打赏</c:if>
        </td>
        <td>${order.goodsId}</td>
