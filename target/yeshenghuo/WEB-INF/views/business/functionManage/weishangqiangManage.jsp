...
<html lang="zh-CN">
			...
			success : function(result) {
				if (result.code != 0) {
					...
					if (result.data.opened == '1') {
						...
						$('#big_screen').href = basePath + info.bigScreenUrl;
					} else {
						$('#functionSet').attr('style', 'display:none');
						$('#pushedContent').attr('style', 'display:none');
					}
				}
			}
		});
  ...
  <body style="padding:15px; overflow:auto;">
    ...
    <ul class="nav nav-tabs">
      ...
      <li id="pushedContent">
        <a data-toggle="tab" href="#pushed-content">上墙内容</a>
      </li>
    </ul>
    <div class="tab-content">
      <div class="tab-pane fade in active" id="open-instruction" style="padding:30px 15px;">
        ...
        <ul class="p1">
          ...
          <li style="color:#ff4b69; font-weight:bold;">注：微喵微上墙中的互动收入微喵将收取一定比例的分成。</li>
        </ul>
        <ul class="p2">
          <h4>开通条件</h4>
          <li>1，必须为vip账号</li>
        ...
        <div class="row">
          <button id="openWShQBtn" class="btn btn-success col-xs-2 col-xs-offset-5" onclick="openWShQ()">开通</button>
        </div>
      </div>
      ...
      <div class="tab-pane fade" id="pushed-content"  style="padding:25px 15px;">
        ...
        <table class="table table-bordered">
          ...
          <tr>
            <td>你好</td>
            <td><img src="<%=request.getContextPath()%>/resource/img/me.jpg" width="35" height="35"></td>
            <td>香蕉网络</td>
            <td>男</td>
            <td>2017-02-09 21:43</td>
          ...
          <tr>
            <th colspan="6" style="text-align:right;">
              <button class="btn btn-info btn-sm">全选</button>
              <button class="btn btn-warning btn-sm">加入黑名单</button>
              <button class="btn btn-danger btn-sm">删除</button>
              <button class="btn btn-danger btn-sm">删除全部消息</button>
            </td>
          </tr>
        </table>
      </div>
